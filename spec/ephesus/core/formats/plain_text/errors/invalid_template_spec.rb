...
require 'ephesus/core/formats/plain_text/errors/invalid_template'

RSpec.describe Ephesus::Core::Formats::PlainText::Errors::InvalidTemplate do
  ...
  describe '#message' do
    let(:expected) do
      "invalid template for message #{notification.type.inspect}"
    end
    ...
    context 'when initialized with expected_key: value' do
      ...
      let(:options)      { super().merge(expected_key:) }
      let(:expected) do
        "invalid template for message #{expected_key.inspect}"
      end
